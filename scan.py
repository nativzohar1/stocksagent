...
With NO arguments it scans the entire Nasdaq-100. This is what the GitHub
Action / Dust agent calls:

    python scan.py                       # scans the whole Nasdaq-100
    python scan.py --json results/out.json
...
thesis, RPO/backlog, product-launch catalysts) are returned as "NEEDS_LLM"
for the Dust agent to resolve via the web.
...
def get_nasdaq100(session=None) -> list:
    """Fetch live Nasdaq-100 constituents from Wikipedia; fall back to a
...
SKIP = "SKIP"             # not computable (missing data) — does not auto-reject
...
class TickerReport:
    ...
    def finalize(self):
        ...
        if any(g.status == NO_GO for g in self.gates):
            ...
            self.verdict = "GO_PENDING_THESIS"
...
def pivot_lows(low: pd.Series, k: int = 3) -> list:
    """Prices of local-minimum bars (Low lowest within +/- k window)."""
    vals = low.values
    out = []
...
def horizontal_support(low: pd.Series, current_low: float, current_price: float,
    ...
    """Detect a horizontal support tested >= min_touches times and whether the
    current bar sits on it. Returns (is_on_support, level, touches)."""
...
def stage1_macro(rep, tk, info, etf_cache, session, sector_etf_override):
    rep.add(Gate(
        ...
        criterion="Enabler or expensive-to-replace data moat"))
...
def stage3_technical_floor(rep, hist):
    ...
    try:
        ...
        floor_hit = near_fib or on_support
        ok = floor_hit and buyers_tail and bounced
        if near_fib:
        ...
        rep.add(Gate("3·Floor", "Concrete floor (fib 0.5/0.618 or 2x support)",
                     GO if ok else NO_GO,
                     detail=f"low={c_low:.2f} on {where}; buyers_tail={buyers_tail}",
                     criterion="Daily candle bounced w/ buyers' tail at 0.5/0.618 "
                               "OR a horizontal support tested >= 2x"))
    except Exception as e:
    ...
    try:
        ...
        rep.add(Gate("3·Floor", "Bullish RSI divergence", GO if ok else NO_GO,
                     detail=f"price LL={price_ll}, RSI HL={rsi_hl} "
                            f"(RSI {float(pr.loc[l1]):.1f}->{float(pr.loc[l2]):.1f})",
                     criterion="Price lower-low while RSI higher-low"))
    except Exception as e:
        rep.add(Gate("3·Floor", "Bullish RSI divergence", SKIP, detail=f"error: {e}"))
    try:
        avg_vol = float(vol.tail(21).mean())
        recent = hist.tail(10)
        bounce_idx = recent["Low"].idxmin()
        bounce_vol = float(vol.loc[bounce_idx])
        ratio = bounce_vol / avg_vol if avg_vol else np.nan
        ok = ratio >= 1.5
        rep.add(Gate("3·Floor", "Volume climax", GO if ok else NO_GO,
                     detail=f"bounce-day vol {bounce_vol/1e6:.1f}M vs 21d avg "
                            f"{avg_vol/1e6:.1f}M ({ratio:.2f}x)",
                     value=round(ratio, 2),
                     criterion="Reversal-day volume >= 1.5x monthly avg"))
    except Exception as e:
        rep.add(Gate("3·Floor", "Volume climax", SKIP, detail=f"error: {e}"))
...
def stage5_catalyst(rep, tk, info):
    try:
        ...
        if next_dt is not None:
            ...
            inwin = 15 <= days <= 45
            rep.add(Gate("5·Catalyst", "Next earnings", GO if (0 <= days <= 60) else NO_GO,
                         detail=f"earnings in {days} days ({next_dt.date()})"
                                + ("" if inwin else " — outside ideal 15-45d window"),
                         value=days,
...
def parse_args(argv=None):
    ...
    p.add_argument("--universe", default="nasdaq100",
                   help="Universe when no tickers given (default: nasdaq100).")
